...
const Header = () => {
    ...
    <header className='fixed w-full top-0 bg-black sh h-28 rounded-md z-40'>
        ...
        <ul className={` ${shownav.opacity} small_tablet:visible absolute top-[190%] small_tablet:top-[55%] -translate-y-1/2 left-0 bg-white small_tablet:bg-transparent  sh small_tablet:shadow-none small_tablet:left-[47%] w-full small_tablet:w-[45%] h-52 small_tablet:h-full flex items-center  gap-3 small_tablet:gap-10 small_tablet:justify-start justify-between flex-col small_tablet:flex-row text-black small_tablet:text-white`}>
            <li><NavLink to="Keroumi-fitness/" end className='relative h-10 small_tablet:h-28 af  small_tablet:w-auto flex items-center cursor-pointer'onClick={Hidenav} style={({isActive})=> isActive ? style: null}>Home</NavLink></li>
            <li><NavLink to="Keroumi-fitness/Contact"className='relative h-10 small_tablet:h-28 af flex items-center cursor-pointer' onClick={Hidenav} style={({isActive})=> isActive ? style: null}>Contact Us</NavLink></li>
            <li><NavLink to="Keroumi-fitness/About"className='relative h-10 small_tablet:h-28 af flex items-center cursor-pointer' onClick={Hidenav} style={({isActive})=> isActive ? style: null}>About us</NavLink></li>
            <li><NavLink to="Keroumi-fitness/Store"className='relative h-10 small_tablet:h-28 af flex items-center cursor-pointer' onClick={Hidenav} style={({isActive})=> isActive ? style: null}>Protein<span className='ml-1'>Store</span></NavLink></li>
        </ul>
